...
# USD文件路径
usd_path = "/home/ju.dong/Desktop/franka_description/urdfs/mobile_fr3_duo_v0_2_franka_hand.usd"
...
base_joint_indices = []
for i, name in enumerate(robot.joint_names):
    if any(keyword in name for keyword in ["tmrv0_2_joint", "caster", "rocker_arm"]):
...
def get_velocity_command():
    ...
    if 's' in _pressed:
        linear_vel -= 10.0   # 后退
    if 'a' in _pressed:
        angular_vel -= 5.0   # 左转（逆时针，改为负值）
        if linear_vel == 0:  # 如果没有按W/S，给个小速度辅助转向
            linear_vel = 3.0
    if 'd' in _pressed:
        angular_vel += 5.0   # 右转（顺时针，改为正值）
        if linear_vel == 0:  # 如果没有按W/S，给个小速度辅助转向
            linear_vel = 3.0
...
try:
    while simulation_app.is_running():
            ...
            # 找到转向关节和驱动轮的索引
            steering_indices = []  # tmrv0_2_joint_0, tmrv0_2_joint_2
            drive_indices = []     # tmrv0_2_joint_1, tmrv0_2_joint_3
            ...
            # 设置转向关节的位置目标
            # 注意：转向角需要取反，因为转向关节正方向可能与预期相反
            steering_angle = -angular_vel * 0.1  # 取负号，将角速度命令转换为转向角（弧度）
            ...
            if count % 50 == 0:
                ...
                if lin != 0 or ang != 0:
                    print(f"\n  步数 {count} | 线速度: {lin:+.1f} | 角速度: {ang:+.1f}")
                    print(f"    按键状态: {_pressed}")
                    # 调试：显示驱动轮
                    print(f"    驱动轮:")
                    for idx in drive_indices:
                        print(f"      [{idx}] {robot.joint_names[idx]}: 目标={linear_vel:+.1f}, 实际={robot.data.joint_vel[0, idx].item():+.1f}")
                    # 调试：显示转向关节
                    print(f"    转向关节:")
                    for idx in steering_indices:
                        print(f"      [{idx}] {robot.joint_names[idx]}: 目标={math.degrees(steering_angle):.1f}°, 实际={math.degrees(robot.data.joint_pos[0, idx].item()):.1f}°")
                    # 显示机器人位置和朝向
                    ...
                    cosy_cosp = 1 - 2 * (robot_quat[2] * robot_quat[2] + robot_quat[3] * robot_quat[3])
                    yaw = math.atan2(siny_cosp, cosy_cosp)
                    print(f"    机器人位置: [{robot_pos[0]:.3f}, {robot_pos[1]:.3f}], 朝向: {math.degrees(yaw):.1f}°")

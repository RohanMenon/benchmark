    ...
    *,
    ...
    if disable_utensil_rigid_bodies:
        for name in DEFAULT_STAGE1_OBJECTS:
            set_rigid_bodies_enabled_under(stage, resolve_prim_path(stage, name), False)
    for _ in range(frames):
        app.update()
    return stage
...
def keep_gui_open(app: Any) -> None:
    ...
    print(
        "GUI_OPEN Integration results printed. Close Isaac Sim or press Ctrl+C.",
        flush=True,
    )
    try:
        try:
...
def run_stage1(app: Any, stage: Any, frames: int) -> dict[str, Any]:
    ...
    bean_positions = [get_prim_position(stage, bean_path) for bean_path in bean_paths]
    beans_in_dining = sum(
        1 for position in bean_positions if classify_task3_area(position) == "dining"
    )
    score = score_stage1_table_setup(positions)
    result = stage_result("stage1", score.score, score.max_score, score.score == 5)
    result["objects_in_dining"] = score.score
    result["objects_total"] = score.max_score
    result["objects_in_dining_percent"] = percentage(score.score, score.max_score)
    ...
    result["beans_in_dining_percent"] = percentage(beans_in_dining, len(bean_paths))
    return result
...
def run_stage2(app: Any, stage: Any, frames: int) -> dict[str, Any]:
    ...
    spoon_start = Point3D(head_feed_pose.x, head_feed_pose.y - 0.20, head_feed_pose.z)
    ...
    beans_after_insert = count_beans_on_spoon(stage, bean_paths, insertion_pose)
    ...
    set_stage2_spoon_pose(stage, spoon_path, spoon_body_offsets, final_spoon_position)
    step_app(app, max(frames, 30))
    timeline.stop()
...
def run_stage3(app: Any, stage: Any, frames: int) -> dict[str, Any]:
    ...
    positions = [get_prim_position(stage, bean_path) for bean_path in bean_paths]
    ...
    result["beans_total"] = len(bean_paths)
    result["beans_inside_sphere_percent"] = percentage(beans_inside, len(bean_paths))
...
def run_stage4(app: Any, stage: Any, frames: int) -> dict[str, Any]:
    ...
    result = stage_result("stage4", score.score, score.max_score, score.score == 5)
    result["objects_in_sink"] = score.score
    result["objects_total"] = score.max_score
    result["objects_in_sink_percent"] = percentage(score.score, score.max_score)
    result["objects_passed"] = score.passed
    result["objects_failed"] = score.failed
...
def interpolate_path(start: Point3D, end: Point3D, steps: int) -> list[Point3D]:
    points = []
...
def y_then_x_path(start: Point3D, end: Point3D, *, steps_per_axis: int) -> list[Point3D]:
    midpoint = Point3D(start.x, end.y, end.z)
    return path_from_waypoints([start, midpoint, end], steps_per_segment=steps_per_axis)


def x_then_y_path(start: Point3D, end: Point3D, *, steps_per_axis: int) -> list[Point3D]:
    midpoint = Point3D(end.x, start.y, end.z)
    return path_from_waypoints([start, midpoint, end], steps_per_segment=steps_per_axis)
...
def drive_prim_along_path(
    ...
    *,
    ...
    for point in path:
        set_prim_position(stage, prim_path, point)
    ...
    for tray_point in path:
        ...
        for name, prim_path in object_paths.items():
            ...
            translate_prim_preserving_rotation(
                stage,
                prim_path,
                Point3D(start.x + delta.x, start.y + delta.y, start.z + delta.z),
            )
        step_app(app, frames_per_step)
...
def set_initial_beans_on_spoon(
    ...
    for index, bean_path in enumerate(bean_paths):
        set_prim_position(stage, bean_path, spoon_bean_position(spoon_position, index))
...
def set_stage2_spoon_pose(
    ...
    for body_path, offset in spoon_body_offsets.items():
        set_prim_world_translation_preserving_rotation(
            stage,
            body_path,
            Point3D(point.x + offset.x, point.y + offset.y, point.z + offset.z),
        )
...
def bean_is_on_spoon(stage: Any, bean_path: str, spoon_position: Point3D) -> bool:
...
def count_beans_on_spoon(
    ...
    return sum(
        1 for bean_path in bean_paths if bean_is_on_spoon(stage, bean_path, spoon_position)
    )
...
def set_named_prim_position(stage: Any, name: str, point: Point3D) -> None:
    set_prim_position(stage, resolve_prim_path(stage, name), point)
...
def set_rigid_bodies_enabled_under(
    ...
    for prim in Usd.PrimRange(root):
        ...
        if attr and attr.IsValid():
            ...
            UsdPhysics.RigidBodyAPI(prim).CreateRigidBodyEnabledAttr(enabled).Set(
                enabled
            )

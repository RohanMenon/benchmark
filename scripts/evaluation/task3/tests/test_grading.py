...
def test_kitchen_and_dining_area_classification():
    expect(
        ...
        classify_table_area((-4.28, -1.59), TASK3_KITCHEN_AREA)
        == "inside",
    )
    expect(
...
def test_dining_classification_respects_dining_lower_y_bound():
    ...
    expect(
        ...
        classify_table_area((0.0, 2.0), kitchen_area=kitchen, dining_area=dining)
        == "outside",
    )
    expect(
        "non-overlapping dining region still works",
        classify_table_area((0.0, 4.5), kitchen_area=kitchen, dining_area=dining)
        == "dining",
    )
...
def test_stage4_scores_utensils_overlapping_sink_above_tabletop():
    ...
    poses = {
        "simple_tray": Bounds2D(sink.x_min, sink.y_min, sink.x_max, sink.y_max),
        "bowl2": Bounds2D(
            sink.x_min - 0.05,
            sink.y_min,
            sink.x_min + 0.05,
